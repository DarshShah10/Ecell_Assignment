...
const Profile = () => {
  ...
  // Function to generate random floating shapes
  const generateShapes = (count, shapeType, color) => {
    const shapes = [];
      ...
      switch (shapeType) {
        case 'circle':
          shapes.push(
            <div
              key={`circle-${i}`}
              className="absolute w-16 h-16 border-4 rounded-full opacity-20 animate-float"
        ...
        case 'square':
          shapes.push(
            <div
              key={`square-${i}`}
              className="absolute w-16 h-16 border-4 opacity-20 animate-float"
              style={{ ...style, borderColor: color }}
            />
          );
          break;
        case 'triangle':
          shapes.push(
            <div
              key={`triangle-${i}`}
              className="absolute w-0 h-0 border-l-[20px] border-l-transparent border-r-[20px] border-r-transparent border-b-[40px] opacity-20 animate-float"
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
        {generateShapes(20, 'circle', '#FF2D55')}
        {generateShapes(20, 'square', '#FF9500')}
        {generateShapes(20, 'triangle', '#34A853')}
      </div>
      ...
      <div className="relative z-10 text-center mt-16">
        ...
        <div className="relative">
          ...
          <div className="absolute -bottom-8 left-1/2 w-0 h-0 border-l-8 border-r-8 border-b-8 border-transparent border-b-[#45D1C9]" />
        </div>
